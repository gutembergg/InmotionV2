...
export default function CompletedOrder(props:IProps) {
...
console.log("props",props)
  ...
  const CompletedOrderOrdernumber = t("completed-order:CompletedOrderOrdernumber");
  ...
  const CompletedOrderPaiementMethod = t("completed-order:CompletedOrderPaiementMethod");
  const CompletedOrderResumeTitle = t("completed-order:CompletedOrderResumeTitle");
  ...
  const CompletedOrderShippingAddress = t("completed-order:CompletedOrderShippingAddress");
  const CompletedOrderBillingAddress = t("completed-order:CompletedOrderBillingAddress");
  ...
  return (<p>sadf</p>
    ...
    //     <p className="productTitle">{order.currency} {lineItem.total}</p>
    //               </li>
    //             )
    //           })
    ...
    //     <h4>{CompletedOrderTVA} ({order.tax_lines[0].rate_percent}%)</h4>
    //     <p>{order.currency} {order.tax_lines[0].tax_total}</p>
    //     <h4>{CompletedOrderTotal}</h4>
    //     <p>{order.currency} {order.total}</p>
    //   </div>
    ...
    //       <address>
    //         {order.billing.last_name} {order.billing.first_name}<br />
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  ...
  // console.log(slug)
  // const order = await getOrder(slug as number);
// console.log(order)
  return {
    props: {
      order
